#' wRAA performance by player season
#'
#' This function returns a MLB player's weighted Runs Above Average
...
#' @param yearID - Season
#' @return Print wRAA of \code{playerID} during \code{yearID}
...
wRAA_season <- function(playerID = NULL, yearID = NULL, ...) {
  ...
  query <- paste("SELECT SUM(H) as H, SUM([2B]) as DB, SUM([3B]) as TR,SUM(HR) as HR, SUM(AB) as AB, SUM(BB) as BB, SUM(IBB) as IBB, SUM(SF) as SF, SUM(SH) as SH, SUM(HBP) as HBP, SUM(AB + BB + HBP + SH + SF) as PA FROM Batting WHERE playerID = '", playerID, "' AND yearID = '", yearID, "'", sep="")
